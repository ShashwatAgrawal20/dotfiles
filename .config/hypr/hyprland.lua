...
hl.on("hyprland.start", function()
    ...
    hl.exec_cmd("~/.local/bin/battery_reminder")
    hl.exec_cmd("wl-paste --type text --watch cliphist store")
    hl.exec_cmd("wl-paste --type image --watch cliphist store")
    hl.exec_cmd("/usr/lib/polkit-gnome/polkit-gnome-authentication-agent-1")
    hl.exec_cmd("/usr/bin/slack -us")
end)
...
hl.bind(mainMod .. " + SHIFT + B", hl.dsp.exec_cmd("bluetoothctl connect 41:42:90:1C:4B:DE"))
...
hl.window_rule({
    match = { initial_title = "(?i)^.*(open|save|select|choose|upload|export).*$" },
    float = true,
    size = "900 600",
    center = true
})
...
hl.window_rule({
    match = { initial_title = "Untitled - Brave|about:blank - Brave" },
    float = true,
    size = "500 600",
    center = true
})
